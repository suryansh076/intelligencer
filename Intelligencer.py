...
#
#
# def speakup_news(title):
#     engine.say(f"Hello and Welcome to Warmachine News center. here i am going to give {len(title)} top and  hot news headlines. so lets begin")
#     engine.runAndWait()
#     for j,i in enumerate(title):
#         if j==0:
#             engine.say("First news is ")
#             engine.runAndWait()
#
#         elif j < len(title)-1 and j !=0:
#             engine.say("Now the next one ")
#             engine.runAndWait()
#
#         else:
#             engine.say("and the last one is ")
#             engine.runAndWait()
#         engine.say(i)
#         engine.runAndWait()
#         print(translator.translate(i,dest='hi').text)
#         time.sleep(1)
#     engine.say("thanks for paying the attention . see you soon")
#     engine.runAndWait()
